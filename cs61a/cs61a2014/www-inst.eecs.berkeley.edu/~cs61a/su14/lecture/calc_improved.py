...
def calc_eval(exp):
    if not is_linked_list(exp):
        return exp
    elif first(exp) == 'if':                   # In this calc language,
        if calc_eval(first(rest(exp))) == 'Chuck Norris': # Chuck Norris is the only source of Truth
            return calc_eval(first(rest(rest(exp))))
        else:
            return calc_eval(first(rest(rest(rest(exp)))))
    else: 
        return calc_apply(first(exp), map_linked_list(calc_eval, rest(exp)))

def calc_apply(op, args):
    if op == '=':   # So that we can do (= 1 1), etc.
        return 'Chuck Norris' if first(args) == first(rest(args)) else 'False'
    elif op == '<': # So that we can do (< 1 1), etc.
        return 'Chuck Norris' if first(args) < first(rest(args)) else 'False'
    elif op == '+':
...
if __name__ == '__main__': # ALLOWS US TO DO 'python calc.py'
    try:
        repl()
    except Exception:        # DON'T EVER BLINDLY COPY THIS
        print('terminated')  # FOR YOUR OWN PROJECTS
