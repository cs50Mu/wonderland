...
class Link:
    ...
    def __add__(self, other):
        ...
        new_rest = self.rest + other # self.rest.__add__(other)
        return Link(self.first, new_rest)
...
class Tree:
    ...
    def max_tree(self):
        ...
        -1
        """
        maximum_so_far = self.datum
        for child in self.children:
            maximum_so_far = max(maximum_so_far, child.max_tree())
...
class BinaryTree:
    ...
    def __iter__(self):
        ...
        if self.left is not None:
            left_iterator = self.left.__iter__()
            for thing in left_iterator:
                yield thing
        yield self.datum
        if self.right is not None:
            right_iterator = self.right.__iter__()
            for thing in right_iterator:
                yield thing
...
def memo_fib(n):
    ...
    if n not in fib_cache:
        fib_cache[n] = memo_fib(n-1) + memo_fib(n-2)
    return fib_cache[n]
