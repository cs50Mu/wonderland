...
(define (loop2)
  ...
  (let ((exp (read)))
    ;; Stop if the user types in Ctrl-D (EOF)
    (if (not (eof-object? exp))
	(begin (print (eval (fix-pet-peeve exp)))
	       (loop2)))))
...
; (infix 1 + 2)
; (infix (infix 48 / 2) * (infix 9 + 3))
; (* (infix 48 / 2) (infix 9 + 3))
(define (infix? exp)
  (and (list? exp)
       (equal? (car exp) 'infix)
       (= (length exp) 4)))

(define (infix-operator exp)
  (caddr exp)) ; Or (list-ref exp 2)

(define (infix-operands exp)
  (list (cadr exp) (cadddr exp)))

(define (fix-infix exp)
  (cond ((infix? exp)
	 (cons (fix-infix (infix-operator exp))
               (map fix-infix (infix-operands exp))))
	((pair? exp)
	 (map fix-infix exp))
	(else exp)))
...
(define (loop3)
  ...
  (flush)
  (let ((exp (read)))
    (let ((no-peeve (fix-pet-peeve exp)))
      (let ((no-infix (fix-infix exp)))
        (if (not (eof-object? exp))
            (begin (print (eval no-infix))
                   (loop3)))))))
