...
#@view('blogs.html')  # 绑定html页面
#@get('/') # 绑定url
#def index():
##    blogs = Blog.find_all()
#    user = User.find_first('where name=?', 'admin@example.com')
...
def make_signed_cookie(id, password, max_age):
    # build cookie string by: id-expire-md5
...
def api_get_blogs():
    ...
    if format == 'html':
        for blog in blogs:
            blog.content = markdown2.markdown(blog.content) # 此处应有误，根本没有import markdown2
