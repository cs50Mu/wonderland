...
def view(path):
    def _decorator(func):  # func为装饰器要装饰的函数
...
class WSGIApplication(object):
    ...
    def run(self, port=9000, host='127.0.0.1'):
        from wsgiref.simple_server import make_server   # Python自带的服务器示例
    ...
    def get_wsgi_application(self,debug=False):
        ...
        def fn_route():
            request_method = ctx.request.request_method # 获取请求的方法
            path_info = ctx.request.path_info    # 获取请求的url
            if request_method == 'GET':
                fn = self._get_static.get(path_info, None)  # 寻找是否有对应url添加的响应规则
                if fn:
                    return fn()
