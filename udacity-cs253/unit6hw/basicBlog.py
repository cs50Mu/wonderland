...
class MainPage(BaseHandler):
    def get(self):
        global MqueryTime
        if memcache.get('posts'):
            posts= memcache.get('posts')
            currentTime = time.time()
            interval = int(currentTime - MqueryTime)
        else:
            posts=db.GqlQuery("select * from Blogdb order by created desc")
            MqueryTime = time.time()
            posts = list(posts)
            memcache.set('posts',posts)
            interval = 0
        self.render('mainpage.html',posts=posts,interval=interval)
...
class PostPage(BaseHandler):
    def get(self,post_id):
        global PqueryTime
        if memcache.get('post'):
            post= memcache.get('post')
            currentTime = time.time()
            interval = int(currentTime - PqueryTime)
        else:
            post = Blogdb.get_by_id(int(post_id))
            PqueryTime = time.time()
        ...
        if not post:
            ...
            self.render('permalink.html',post = post,interval=interval)

class Flush(BaseHandler):
    def get(self):
        memcache.delete_multi(['posts','post'])
        MqueryTime = 0
        PqueryTime = 0
        self.redirect('/')
